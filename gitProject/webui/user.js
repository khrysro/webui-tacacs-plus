...
function getUserResults() {
		...
		if (xmldoc.getElementsByTagName('user')[0].firstChild.nodeValue == 1) {
			...
			resultForm.shell = xmldoc.getElementsByTagName('homedir')[0].firstChild.nodeValue;
		else
			resultForm.shell = '';
        }
}
...
function _group(pid, uid) {
	...
	document.userform.action = "?module=user";
	document.userform.submit();
}
...
function _open_contact(uid, obj) {
	if (uid.value) {
		...
		showMe();
		document.getElementById("_nodeframe").src = "contact.php?_ret=5&id="+this.id+"&uid="+uid.value;
	}
}
